...
def api_central_parametros_buscar():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE table_name IN ('cadastro_produtos', 'estoque_posicao_atual', 'situacao_compra_itens', 'padrao_compra_item')
        """)
        ...
        for p in produtos_base:
            ...
            produto['padrao_compra'] = None  # Loja destino do padrao de compra
            ...
            # Buscar padrao de compra
            if 'padrao_compra_item' in tabelas_existentes:
                try:
                    if cod_empresa_filter:
                        cursor.execute("""
                            SELECT cod_empresa_destino FROM padrao_compra_item
                            WHERE codigo::text = %s AND cod_empresa_venda = %s
                            LIMIT 1
                        """, [produto['cod_produto'], cod_empresa_filter])
                    else:
                        cursor.execute("""
                            SELECT cod_empresa_venda, cod_empresa_destino FROM padrao_compra_item
                            WHERE codigo::text = %s
                            LIMIT 5
                        """, [produto['cod_produto']])
                    ...
                    if padroes:
                        if cod_empresa_filter:
                            produto['padrao_compra'] = padroes[0]['cod_empresa_destino']
                        else:
                            # Sem filtro de loja, mostrar lista resumida de padroes
                            padroes_list = [f"{p['cod_empresa_venda']}→{p['cod_empresa_destino']}" for p in padroes]
                            produto['padrao_compra'] = ', '.join(padroes_list)
                except:
                    pass
...
# ============================================================================
# APIS DE PADRAO DE COMPRA
# ============================================================================

@app.route('/api/padrao-compra/buscar', methods=['GET', 'POST'])
def api_padrao_compra_buscar():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Buscar padroes para lista de codigos
...
@app.route('/api/padrao-compra/estatisticas', methods=['GET'])
...
@app.route('/api/padrao-compra/itens-sem-padrao', methods=['GET'])
...
@app.route('/api/padrao-compra/processar-pedidos', methods=['POST'])
def api_padrao_compra_processar_pedidos():
    """
    Processa pedidos aplicando logica de padrao de compra.

    Recebe lista de itens ja calculados e reagrupa por loja destino
    considerando o padrao de compra de cada item/loja.

    Request JSON:
    {
        "itens": [...],  // Lista de itens com codigo, cod_empresa, demanda_prevista
        "lead_times": {cod_empresa: lead_time_dias},  // Lead times por loja (opcional)
        "agrupar_por_fornecedor": true  // Se true, agrupa por fornecedor > destino
    }

    Returns:
        JSON com pedidos agrupados por destino (e opcionalmente por fornecedor)
    """
    try:
        ...
        if not itens:
            return jsonify({
                'success': False,
                'erro': 'Nenhum item fornecido para processar'
            }), 400

        print(f"\n{'='*60}")
        print("PROCESSAMENTO COM PADRAO DE COMPRA")
        print(f"{'='*60}")
        print(f"  Itens recebidos: {len(itens)}")

        conn = get_db_connection()

        # Processar com padrao de compra
        calculador = PadraoCompraCalculator(conn)
        resultado = calculador.processar_itens_com_padrao(itens, lead_times)

        # Se solicitado, agregar por fornecedor
        if agrupar_por_fornecedor and resultado['sucesso']:
            agregado = agregar_pedidos_por_destino_fornecedor(resultado['pedidos_por_destino'])
        else:
            agregado = None

        # Formatar para Excel (item x destino por linha)
        linhas_excel = formatar_pedido_para_excel(resultado['pedidos_por_destino'])

        conn.close()

        print(f"  Destinos gerados: {resultado.get('total_destinos', 0)}")
        print(f"  Total itens processados: {resultado.get('total_itens', 0)}")
        print(f"  Criticas: {resultado.get('total_criticas', 0)}")
        print(f"{'='*60}\n")

        return jsonify({
            'success': resultado['sucesso'],
            'mensagem': resultado['mensagem'],
            'pedidos_por_destino': resultado['pedidos_por_destino'],
            'agregado_fornecedor_destino': agregado,
            'linhas_excel': linhas_excel,
            'estatisticas': {
                'total_destinos': resultado.get('total_destinos', 0),
                'total_itens': resultado.get('total_itens', 0),
                'dias_transferencia': resultado.get('dias_transferencia', 10)
            },
            'criticas': resultado.get('criticas', []),
            'total_criticas': resultado.get('total_criticas', 0)
        })
...
@app.route('/api/padrao-compra/exportar', methods=['POST'])
...
@app.route('/api/parametros-globais', methods=['GET'])
def api_parametros_globais():
    """Retorna todos os parametros globais do sistema."""
    try:
        conn = get_db_connection()
        ...
        return jsonify({
            'success': True,
            'parametros': df.to_dict('records')
        })

    except Exception as e:
        import traceback
        print(f"[ERRO] api_parametros_globais: {e}")
        traceback.print_exc()
        return jsonify({'success': False, 'erro': str(e)}), 500


@app.route('/api/parametros-globais/<chave>', methods=['PUT'])
def api_parametros_globais_atualizar(chave):
    """Atualiza um parametro global."""
    try:
        dados = request.get_json()
        valor = dados.get('valor')

        if valor is None:
            return jsonify({'success': False, 'erro': 'Valor nao fornecido'}), 400
        ...
        if cur.rowcount == 0:
            conn.close()
            return jsonify({'success': False, 'erro': f'Parametro {chave} nao encontrado'}), 404
        ...
        return jsonify({
            'success': True,
            'mensagem': f'Parametro {chave} atualizado para {valor}'
        })

    except Exception as e:
        import traceback
        print(f"[ERRO] api_parametros_globais_atualizar: {e}")
        traceback.print_exc()
        return jsonify({'success': False, 'erro': str(e)}), 500
