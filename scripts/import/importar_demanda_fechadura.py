# -*- coding: utf-8 -*-
"""Script para importar dados de demanda de Fechadura (SILVANA, STAM, etc)"""
import sys
import os
import csv
...
def main():
    print('='*60)
    print('IMPORTACAO DE DADOS DE DEMANDA - FECHADURA')
    print('(SILVANA, STAM e outros fornecedores)')
    print('='*60)
    print(f'Data/Hora: {datetime.now().strftime("%d/%m/%Y %H:%M:%S")}')

    # Lista de arquivos para importar
    arquivos_vendas = [
        ('demanda_Fechadura_2023.csv', 'Fechadura 2023'),
        ('demanda_Fechadura_2024.csv', 'Fechadura 2024'),
        ('demanda_Fechadura_2025.csv', 'Fechadura 2025'),
        ('demanda_Fechadura_28-01-2026.csv', 'Fechadura Jan/2026'),
    ]
    ...
    for arq, label in arquivos_vendas:
        ...
        if os.path.exists(path):
            ins, upd, err = importar_vendas(path, label)
            total_inseridos += ins
    ...
    print(f'   TOTAL REGISTROS: {total_inseridos + total_atualizados:,}')
    print('='*60)
